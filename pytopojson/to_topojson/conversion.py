...
def convert(geojson, topojson=None, object_name=False, *args, **kwargs):
    if isinstance(geojson, dict):
        ...
        try:
            with open(geojson) as inFile:
                input_dict = load(inFile)
        except:
            input_dict = loads(geojson)

        if not object_name and 'type' in input_dict and hasattr(inFile,'name') \
                and inFile.name.lower().endswith('.geojson'):
